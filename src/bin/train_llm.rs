...
/// Rank-1 parameters and the explicitly configured per-neuron `[H,Q]` gate
/// matrices use identical Adam moments without decay. The shape exception is
/// semantic to this architecture: `raw_u`, `raw_alpha`, and `raw_rho` all have
/// exactly `[H,Q]`, while learned projections retain decay. This prevents
/// regularization from silently pulling sigmoid/tanh gates toward their
/// central values regardless of the language-model gradient.
#[derive(Clone)]
struct AdamWSelectiveDecay {
    decayed: AdamW,
    no_decay: AdamW,
    per_neuron_gate_shape: [usize; 2],
}

impl<B: Backend> SimpleOptimizer<B> for AdamWSelectiveDecay {
    ...
    fn step<const D: usize>(
        ...
        let is_per_neuron_gate =
            D == 2 && shape.dims::<D>().as_slice() == self.per_neuron_gate_shape;
        if D == 1 || is_per_neuron_gate {
...
struct LogEvent<'a> {
    ...
    state_alpha_min: Option<f32>,
    state_alpha_mean: Option<f32>,
    state_alpha_max: Option<f32>,
...
fn main() -> Result<(), AnyError> {
    ...
    println!(
        "architecture: MHAR heads={}, tied_depth_query={}, delta_wide={}, u0={:.3}, alpha0={:.3}, depth_norm={}, cq_rho0={:.5}, TBPTT={}x{} tokens/lane",
        model.attention_residual_heads(),
        config.model.attn_residual_tied,
        config.model.gated_neuron_state,
        config.model.gated_neuron_state_initial_update,
        config.model.gated_neuron_state_initial_injection,
        config.model.normalize_each_depth,
    ...
    let mut optimizer = OptimizerAdaptor::from(AdamWSelectiveDecay {
        decayed: adam.clone().build(),
        no_decay: adam.with_weight_decay(0.0).build(),
        per_neuron_gate_shape: [
            config.model.heads,
    ...
    O: Optimizer<Bdh<TrainingBackend>, TrainingBackend>,
{
    ...
    loop {
        ...
        if state.optimizer_step.is_multiple_of(config.log_every_steps) {
            ...
            let rho = tensor_statistics(model.cq_retention_probabilities())?;
            let base_u = tensor_statistics(model.base_state_update_probabilities())?;
            let state_alpha = tensor_statistics(model.state_injection_strengths())?;
            ...
            let event = LogEvent {
                ...
                state_alpha_min: state_alpha.minimum,
                state_alpha_mean: state_alpha.mean,
                state_alpha_max: state_alpha.maximum,
...
fn tensor_statistics<const D: usize>(
    probabilities: Option<Tensor<TrainingBackend, D>>,
) -> Result<ParameterSummary, AnyError> {
    let Some(probabilities) = probabilities else {
        return Ok(ParameterSummary::default());
    };
...
mod tests {
    ...
    #[test]
    fn adamw_exempts_rank_one_and_per_neuron_gate_parameters_from_decay() {
        type OptimBackend = NdArray<f32>;
        let device = Default::default();
        let config = AdamWConfig::new().with_weight_decay(0.1);
        let optimizer = AdamWSelectiveDecay {
            decayed: config.clone().build(),
            no_decay: config.with_weight_decay(0.0).build(),
            per_neuron_gate_shape: [2, 3],
        };
        let rank_one = Tensor::<OptimBackend, 1>::ones([2], &device);
        let per_neuron_gate = Tensor::<OptimBackend, 2>::ones([2, 3], &device);
        let ordinary_matrix = Tensor::<OptimBackend, 2>::ones([3, 2], &device);
        let (rank_one, _) = optimizer.step(0.1, rank_one, Tensor::zeros([2], &device), None);
        let (per_neuron_gate, _) =
            optimizer.step(0.1, per_neuron_gate, Tensor::zeros([2, 3], &device), None);
        let (ordinary_matrix, _) =
            optimizer.step(0.1, ordinary_matrix, Tensor::zeros([3, 2], &device), None);
        assert_eq!(
            rank_one.into_data().to_vec::<f32>().unwrap(),
            vec![1.0, 1.0]
        );
        assert_eq!(
            per_neuron_gate.into_data().to_vec::<f32>().unwrap(),
            vec![1.0; 6]
        );
        ordinary_matrix.into_data().assert_approx_eq::<f32>(
            &TensorData::new(vec![0.99_f32; 6], [3, 2]),
